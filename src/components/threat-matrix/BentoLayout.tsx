  ...
  return (
    ...
    >
      <div className="absolute top-0 left-0 right-0 h-1 bg-white/5">
        <motion.div 
          key={activeIndex}
          className="h-full bg-gradient-to-r from-[#d97757] to-[#6a9bcc]"
          initial={{ width: "0%" }}
      ...
      <div className="flex flex-col md:flex-row relative z-10 min-h-[300px]">
        ...
        <div className="w-full md:w-7/12 p-8 md:p-10 flex flex-col justify-center relative overflow-hidden">
          <motion.div 
            className="absolute top-0 left-0 w-full h-[1px] bg-[#6a9bcc]/30"
            animate={{ y: ['0px', '300px', '0px'] }}
            transition={{ duration: 4, repeat: Infinity, ease: 'linear' }}
          />
          
          <h3 className="text-[10px] uppercase tracking-[0.2em] mb-6 font-mono flex items-center gap-2 text-[#6a9bcc]">
            ...
            >
              <div className="flex flex-col gap-6">
                <div className="flex items-start gap-5">
                  <div className="p-4 rounded-2xl bg-[#6a9bcc]/10 text-[#6a9bcc]">
                    {activeData.countermeasure.icon}
                  </div>
                  <div>
                    <h4 className="text-3xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-[#d97757] to-[#6a9bcc] mb-2">
                      {activeData.countermeasure.title}
                    </h4>
                    <p className="text-[#faf9f5]/50 text-lg leading-relaxed">
                ...
                <div className="grid grid-cols-2 gap-4 pt-4">
                  {activeData.countermeasure.metrics.map((metric, i) => (
                    <div key={i} className="flex flex-col">
                       <span className="text-[10px] text-[#faf9f5]/30 uppercase tracking-widest font-mono mb-1">Impact {i + 1}</span>
                       <span className="font-bold text-xl text-[#6a9bcc]">{metric}</span>
                    </div>
                  ))}
                </div>
              </div>
